...
ap.add_argument("--engine",  choices=["hf", "vllm"], default="hf", help="★ 生成引擎：hf = transformers generate；vllm = vLLM 离线引擎（要 --hf-dir，在装了 vLLM 的 venv 里跑）")
ap.add_argument("--hf-dir",  default=None, help="vLLM 用：export_hf.py 导出的目录")
ap.add_argument("--gpu-mem", type=float, default=0.6, help="vLLM 占显存比例")
ap.add_argument("--tool",    action="store_true",
                help="★ 计算器：提示词加工具说明，生成走 calc_tool.generate_with_tools（停、注入、续）。评工具版模型必须开")
ap.add_argument("--ask",     action="store_true",
                help="★ 臂 A：= --tool + 提示词加求助说明 + harness 开 <ask>（问 DeepSeek，要先 export DEEPSEEK_API_KEY）；记每条求助次数，输出名带 _ask")
ap.add_argument("--ask-model", default="deepseek-reasoner", help="求助工具背后的专家；探针用 deepseek-chat（弱，硬题 1/10），看模型抓不抓得住错答案；输出名带 _chat")
ap.add_argument("--prompt-set", default=None, choices=["train", "heldout", "all", "r1"],
                help="★ 用 prompts_cd 的改写模板（每题固定一个，按题号轮流），--tool/--ask 决定说明给不给；不传 = 原 R1 模板。输出名带 _pSET")
ap.add_argument("--max-asks", type=int, default=2, help="每条最多求助几次（提示词里的次数跟着变）")
...
if args.ask:
    ...
    import deepseek_tool as _DS; _DS.get_key()                # 没设 DEEPSEEK_API_KEY 就在这儿停，别等采样到一半
...
def out_name():
    ...
    if args.tool:
        stem += "_tool"
    if args.ask:
        stem += "_ask" + ("" if args.ask_model == "deepseek-reasoner" else "_" + args.ask_model.replace("deepseek-", ""))
    if args.prompt_set:
        stem += f"_p{args.prompt_set}"
    if args.shard:
...
TOOLST = dict(calls=0, err=0, fake=0, asks=0, rounds=0, batches=0)
TOOLREC = []                                              # 跟 rec 对齐：工具模式每条 (calls, err, fake, asks)，落盘给 --merge 用
...
if args.merge:
    ...
    for sh in shards:
        ...
        for r in sh["records"]:
            ...
            if "calls" in r:
                TOOLREC.append((r["calls"], r["err"], r["fake"], r.get("asks", 0)))
                TOOLST["calls"] += r["calls"]; TOOLST["err"] += r["err"]; TOOLST["fake"] += r["fake"]; TOOLST["asks"] += r.get("asks", 0)
        ts = sh.get("toolst", {})
    ...
    if args.engine == "vllm":                                  # ★ vLLM：权重从 --hf-dir 装，不载 HF 模型
        assert args.hf_dir, "--engine vllm 要 --hf-dir（export_hf.py 导出的目录）"
        print(f"载入 vLLM ← {args.hf_dir} …", flush=True)
        backend = calc_tool.make_backend("vllm", tok=tok, hf_dir=args.hf_dir, gpu_mem=args.gpu_mem, max_model_len=args.max_new + 400)
        args.bs = max(args.bs, len(P) * args.samples)          # 一次全交给它，连续批处理自己调度
    else:
        ...
        backend = calc_tool.make_backend("hf", model=model, tok=tok, dev=dev, gen_bs=args.bs)

    # ── 采样 ──
    if args.prompt_set:                                          # ★ 改写模板：题 i 固定用集合里第 i 个（轮流），措辞也按题号定，8 次采样同一份提示词
        import prompts_cd as PC
        ids = PC.SETS[args.prompt_set]
        MK = lambda nums, target, i: PC.render(ids[i % len(ids)], nums, target, tool=args.tool, ask=args.ask, max_asks=args.max_asks,
                                               tool_style=i % 3, ask_style=(i // 3) % 3)
    elif args.ask:
        MK = lambda nums, target, i: calc_tool.ask_prompt(nums, target) if args.max_asks == 2 else \
            calc_tool.tool_prompt(nums, target).replace("Show your work in", calc_tool.ask_hint(args.max_asks) + "Show your work in")
    else:
        MK0 = plain_prompt if args.plain else (calc_tool.tool_prompt if args.tool else CD.make_prompt)
        MK = lambda nums, target, i: MK0(nums, target)
    flat = [(i, MK(p["nums"], p["target"], i))
    ...
    with torch.no_grad():
        for s in range(0, len(flat), args.bs):
            ...
            if args.tool:                                   # ★ 计算器 harness：停、注入、续
                res = calc_tool.generate_with_tools(backend, tok, [p for _, p in chunk], max_new=args.max_new,
                                                    temp=args.temp, gen_bs=len(chunk), dev=dev, progress=(args.engine == "vllm"),
                                                    ask=args.ask, max_asks=args.max_asks,
                ...
                for (pi, _), o in zip(chunk, res):
                    ...
                    TOOLST["calls"] += o["n_calls"]; TOOLST["err"] += o["n_err"]; TOOLST["fake"] += o["fake"]; TOOLST["asks"] += o.get("n_asks", 0)
                    TOOLREC.append((o["n_calls"], o["n_err"], int(o["fake"]), o.get("n_asks", 0)))
                ...
                print(f"  {done:>5}/{len(flat)}  {el/60:>5.1f} 分  {ntok/el:>6.0f} tok/s  剩 {el/done*(len(flat)-done)/60:>5.1f} 分"
                      f"  调用/条 {TOOLST['calls']/done:.1f}  报错 {TOOLST['err']}  假结果 {TOOLST['fake']}"
                      + (f"  求助 {TOOLST['asks']}" if args.ask else ""), flush=True)
                continue
            res = calc_tool.plain_generate(backend, tok, [p for _, p in chunk], max_new=args.max_new, temp=args.temp)   # 停在 "\nUser:"
            for (pi, _), o in zip(chunk, res):
                ntok += o["L"]
                te, cont, L, Le = o["txt"], o["cont"], o["L"], o["L_eff"]
                r, d = CD.reward(te, P[pi]["nums"], P[pi]["target"])
...
json.dump({"model": args.model, "ckpt": args.ckpt, "n": len(P), "n_samples": args.samples,
           ...
           "time": T, "ntok": ntok, "toolst": TOOLST, "prompt_set": args.prompt_set, "max_asks": args.max_asks,
           # ★ 现在存文本了 —— 之前只存分数，回头想查措辞查不了
           "records": [dict(pi=pi, r=r, d=d, L=L, L_eff=Le, reflect=rf, search=sf, cont=ct, txt=t,
                            **(dict(calls=tr[0], err=tr[1], fake=tr[2], asks=tr[3]) if tr else {}))
...
print(f"""
  ...
  ── 工具 ──""" + (f"""
    ...
    ★ 求助：{TOOLST['asks']} 次 / {len(rec)} 条 = {TOOLST['asks']/len(rec):.2f} 次/条   有求助的轨迹 {sum(1 for t in TOOLREC if t[3] > 0)} ({100*sum(1 for t in TOOLREC if t[3] > 0)/len(rec):.1f}%)   求助过的对 {np.mean([r[3]['correct'] for r, t in zip(rec, TOOLREC) if t[3] > 0]) if any(t[3] > 0 for t in TOOLREC) else 0:.3f}   没求助的对 {np.mean([r[3]['correct'] for r, t in zip(rec, TOOLREC) if t[3] == 0]) if any(t[3] == 0 for t in TOOLREC) else 0:.3f}""" if (args.ask or any(t[3] > 0 for t in TOOLREC)) else "") + f"""
  ── 长度 ──
