...
**做了什么。** 学习了 Stanford CS336 和 Stanford CS329A，在四张 4090 上从零预训练两个英文模型（95M、201M），做 SFT，然后转到 RL：先在 GSM8K 上复现 GRPO，再用 Countdown 数字游戏追 DeepSeek-R1 的「顿悟」，一路把这个任务的 pass@1 从 0.02 抬到 0.75，开专家出口后 0.98；接着做第一个有状态的环境（猜数字），最后进入编程 agent：修 bug、拼包、从头写真实模块、拿走测试让模型自己验证，共四级。
...
**为什么停在 1.5B。** 先说约束：四张 24 GB 的 4090，全参 RL 加同卡 vLLM，1.5B 是能放下的上限，4B 就得换 LoRA。规模变不了，于是把它当常量，只变能变的那些量：同一个底模、同一把秤、一次只变一个量，差异才能归因。这不是缩放研究，也不是小模型的成绩对比；所有结论限定在这个规模和这些任务上，「1.5B 手里不值分」「装不进」都是对这个容量的描述，换容量要重新量。
...
- **规模。** 硬件把底模规模定在 1.5B 以下，四张 24 GB 卡放不下更大的全参 RL；所有实验因此在一个规模上做，不能说明规律随规模怎么变，「不值分」「装不进」都是对这个容量的描述。
