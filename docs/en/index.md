...
| 5 | 09-02 | Countdown 3-number, R1-Zero reproduction | Qwen2.5-1.5B Base | .035 → .405, length 186 → 48 | Shortcut; conditional policy binding (条件性策略绑定) |
...
| 8 | 09-07 to 09-08 | Calculator tool, Arm T, the first harness | same | SFT .506 → .710; RL .711 | ⑮ once the parts are fixed, RL has only shaping-away (塑走) left |
| 9 | 09-08 | Arm S, subtractive scale (秤做减法) + pool screening + anchor | same | .754; CD-3 ceiling .92 → 1.00 | ⑯ RL moves within the reachable set (可达集) |
| 10 | 09-10 to 09-11 | Arm A, asking an expert with a budget (限量问专家) | same | with help .979, hard problems .946; wording change .966 → .036 | ⑰ the method binds to the prompt wording; the expert = all the gain on hard problems |
...
11. E number guessing: the smallest agent environment; pure RL grows binary search from 4%, curriculum beats training directly on the hard setting (硬地), seeding is best, N=1000 relies on "writing down the interval" → explicit state comes from the textbook, the harness is generalized → ready for real tasks.
...
13. ① bug fixing: get the loop, the sandbox, and the scale running; zero-shot ≈ 0 → a cold start is required; SFT +.55, RL +.12; pass@32 unchanged; strict scale (严秤) + cheating probe → ask what happens when the path gets longer.
...
4. **Textbook.** Source: program teacher (程序老师), distillation, or self-sampling with filtering; the list of what to teach comes from the probe's failure modes; held-out templates do not enter the textbook.
...
7. **Conclusion (定论), prediction reconciliation (预测对账), debts (欠债).** The debts are the source of the next round's step 4.

### 2.2 Evaluation protocol

- **pass@1**: temperature 1, 8 samples per problem, 100 out-of-pool (池外) problems, reported as the mean over 800 samples. 1σ is about 0.03 to 0.04; any difference smaller than that is read as flat.
...
**Probe.** The two ②-B models enter the weak room (弱房间): RL .295, SFT .240; first version .327 / .248; self-test rate .32 / .26; false positives .19; per fix round −.028. Predicted .10 to .15, actual .30, underestimated by a factor of two: tests only affect the fix loop, the first version does not depend on them.
