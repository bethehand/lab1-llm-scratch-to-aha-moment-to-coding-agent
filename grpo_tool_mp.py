...
import calc_tool                                  # ★ 计算器 harness（--tool）：停、注入、续、mask
...
ap.add_argument("--fake-pen", type=float, default=0.2, help="模型自己写 <result> 扣多少")
...
# ══════════════════════════════════════════════════════════════
@torch.no_grad()
def _finish(pi, ids, txt, cont, spans, n_calls, n_err, fake, probs, max_new):
    r, d = score(probs[pi], txt)
    if args.judge == "v2" and probs[pi]["task"] == "cd" and d["correct"] > 0:
        r -= J2.PEN["len_ok"] * len(ids) / max_new
    ...
    return dict(pi=pi, task=probs[pi]["task"], ids=ids, txt=txt, r=r, d=d,
                ...
                nannot=na, annot_ok=nok, cont=cont, spans=spans, n_calls=n_calls, n_err=n_err, fake=fake)
    ...
    if args.tool:                                                   # ★ CD 题走 harness，GSM 照旧
        ...
        for s in range(0, len(flat_cd), args.gen_bs):
            ...
            res = calc_tool.generate_with_tools(raw, tok, [pr for _, pr in chunk], max_new=max_new, temp=temp,
                                                max_calls=args.max_calls, gen_bs=args.gen_bs, dev=DEV)
            for (pi, _), o in zip(chunk, res):
                out.append(_finish(pi, o["ids"], o["txt"], o["cont"], o["spans"], o["n_calls"], o["n_err"], o["fake"], probs, max_new))
...
for step in range(step0, args.steps):
    ...
    recs = [(pc[x["pi"]] + x["ids"], len(pc[x["pi"]]), a, calc_tool.response_mask(len(x["ids"]), x.get("spans", [])))
            for x, a in zip(roll, adv) if len(x["ids"]) > 0]
    my_tok = sum(sum(m) for _, _, _, m in recs)
    ...
    for s in range(0, len(recs), args.micro):
        ...
        ids, attn, rm = pad_batch([(f, n, m) for f, n, _, m in mb])
        A = torch.tensor([a for _, _, a, _ in mb], device=DEV).unsqueeze(1)
        rlp = None
        if ref is not None:
            with torch.no_grad():
